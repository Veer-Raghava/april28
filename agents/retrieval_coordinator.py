...
class RetrievalCoordinatorAgent(BaseAgent):
    ...
    async def run(self) -> None:
        ...
        for record in pending:
            ...
            for candidate in candidates:
                ...
                triage_result = self._triage_url(
                    url,
                    triage_keywords,
                    snippet_hint=str(candidate.get("snippet", "")),
                )
    ...
    def add_more_sources(self, new_queries: list[str], topic: str, record: dict) -> int:
        ...
        for r in results:
            ...
            result = self._triage_url(url, triage_kw, snippet_hint=str(r.get("snippet", "")))
    ...
    def _is_trusted_science_url(self, url: str) -> bool:
        host = urlparse(url).netloc.lower()
        return any(dom in host for dom in _TRUSTED_SCIENCE_DOMAINS)

    def _triage_url(self, url: str, keywords: list[str], snippet_hint: str = "") -> str:
        if url.lower().endswith(".pdf") or "/pdf/" in url.lower():
            return "pdf"
        try:
            kw_norm = [str(k).lower() for k in keywords if str(k).strip()]
            ...
            if cl:
                try:
                    if int(str(cl).replace(",", "").strip()) < 2000:
                        return "too_short"
                except Exception:
                    pass
            ...
            if not resp:
                if self._is_trusted_science_url(url):
                    return "pass"
                return "error"
            ...
            if not snippet and snippet_hint:
                snippet = snippet_hint.lower()
            ...
            if snippet and not any(kw in snippet for kw in kw_norm):
                if self._is_trusted_science_url(url):
                    return "pass"
                return "no_content"
